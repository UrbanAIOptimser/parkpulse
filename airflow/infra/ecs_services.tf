resource "aws_ecs_service" "web_server_service" {
    ...
    depends_on = [
        aws_db_instance.postgres_metadata,
        aws_elasticache_cluster.celery_backend,
        aws_alb_listener.airflow_web_server,
    ]
    tags = {
    Name = "${var.project_name}-${var.stage}-ecs_service-webservice"
   }
}
...
resource "aws_ecs_service" "scheduler_service" {
    ...
    tags = {
    Name = "${var.project_name}-${var.stage}-ecs_service-scheduler"
   }
}
...
resource "aws_ecs_service" "workers_service" {
    ...
    tags = {
    Name = "${var.project_name}-${var.stage}-ecs_service-worker"
   }
}
...
resource "aws_ecs_service" "flower_service" {
  ...
  tags = {
    Name = "${var.project_name}-${var.stage}-ecs_service-flower"
  }
}
