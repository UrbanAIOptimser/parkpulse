...
[
  {
    ...
    "image": ${replace(jsonencode("${aws_ecr_repository.docker_repo.repository_url}:${var.image_version}"), "/\"([0-9]+\\.?[0-9]*)\"/", "$1")} ,
    "essential": true,
    "portMappings": [
      {
        "containerPort": 8080,
    ...
    "environment": [
      ...
      {
        "name": "POSTGRES_HOST",
        "value": ${replace(jsonencode(aws_db_instance.postgres_metadata.address), "/\"([0-9]+\\.?[0-9]*)\"/", "$1")}
      },
      {
        "name": "POSTGRES_PORT",
        "value": "5432"
      ...
      },
      {
          ...
          "value": ${replace(jsonencode(random_string.metadata_password.result), "/\"([0-9]+\\.?[0-9]*)\"/", "$1")}
      },
      {
          "name": "POSTGRES_DB",
          "value": "${var.project_name}"
